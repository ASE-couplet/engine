...
def experiment1():
    ...
    for test in tests:
        ...
        for m, alignment in zip([model, model_alignment], [False, True]):
            ...
            result = [t[0] for t in m.wv.most_similar(positive=[str(test, "utf-8")])]
...
def refine(ch_rhyme, ch, alignment=False, topn=50):
    ...
    rhyme = rdict.get_rhyme(str(ch_rhyme, "utf-8"))
    result = [t[0] for t in model.wv.most_similar(positive=[str(ch, "utf-8")], topn=topn)]
    filtered_result = [ch for ch in result if ch in ch2int]
